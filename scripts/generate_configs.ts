...
// Basic Move Mapping (Copied/Shared logic)
...
const getGenericMoves = (types: string[], isPhysical: boolean): string[] => {
    ...
    while (moves.length < 4) moves.push('Protect'); // Fill up to 4
...
const main = () => {
    ...
    // Existing configs to preserve (optional, but good practice to avoid overwriting hand-tuned ones)
    // Actually, user wants ALL. Providing a "base" for all is better.
    // We will prefix generated ones or just overwrite?
    // Let's Skip if file exists to preserve manual edits.
    const existingFiles = new Set(fs.readdirSync(outputDir).map(f => f.toLowerCase()));

    let count = 0;
    for (const species of gen9.species) {
        if (species.nfe) continue; // Skip NFE

        const id = toID(species.name);
        const filename = `${id}.json`;

        if (existingFiles.has(filename)) {
            console.log(`Skipping existing: ${species.name}`);
            continue;
        }
