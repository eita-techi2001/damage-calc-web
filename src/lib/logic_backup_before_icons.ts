...
export async function calculateDamageForConfig(baseUserPoke: UserPokemonConfig, globalField?: GlobalFieldState, opponentOverrides?: UserPokemonConfig[]) {
    const calc = new DamageCalculator();
    const explorer = new LineExplorer();
    const gen = 9;
    const baseFieldArgs: any = {}; // Placeholder for future manual field config
    ...
    let targets: any[] = opponentOverrides && opponentOverrides.length > 0
        ? opponentOverrides
        : [...MetaPokemons];

    // Note: opponentOverrides here is really just "the list of opponents to use".
    // I kept the variable name in signature to avoid changing the function header in this simplified step,
    // or I can change it safely since I updated the caller in actions.ts.
    // logic.ts exports this function. actions.ts uses it.
    // I will assume the name passed from actions.ts is just the 3rd arg.

    // Check if we need to process labels for legacy reasons?
    // If the frontend sends objects, they should already have 'extraLabel' if they are custom.
    // But let's be safe: If an object doesn't have extraLabel and isn't a known meta one (how to tell?), we might want to label it.
    // BUT user wanted "Manage List".

    // Simplest: Trust the list.
    ...
    const calculateBerryAdjustedKO = (result: any, minDmg: number, maxDmg: number) => {
        ...
        if (defenderItem === 'Sitrus Berry' || defenderItem === 'オボンのみ') {
            ...
            const simulateHits = (dmg: number) => {
                ...
                for (let i = 1; i <= 9; i++) {
                    ...
                    if (!consumed && currentHP <= sitrusThreshold) {
                        currentHP += sitrusRecovery;
                        consumed = true;
                    }
                }
                return { hits: 9, consumed }; // Cap at 9
            };
    ...
    for (const variant of userVariants) {
        ...
        const userScenarios = [{ isTera: false, label: '' }];
        ...
        for (const userScenario of userScenarios) {
            for (const defender of targets) {
                const defenderScenarios = [{ isTera: false, label: '' }];
